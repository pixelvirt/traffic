...
list_of_vehicles = ["car", "bus", "truck", "motorbike"]
...
# Draw a red line in the middle of the video (640, 0) (640, 1080)


def draw_red_line_real(frame):
...
# downscale by 60%
video_width = int(video_width * 0.6)
video_height = int(video_height * 0.6)
...
frame_skip_interval = 20  # Adjust this value as needed
...
while True:
    # Increment the frame_skip_counter
    frame_skip_counter += 1

    # Skip frames if frame_skip_counter is not 0
    ...
    #video_width = int(frame.shape[1] * scale_percent / 100)
    #viedo_height = int(frame.shape[0] * scale_percent / 100)
    #dim = (video_width, viedo_height)
  
    # resize image
    #frame = cv2.resize(frame, dim, interpolation = cv2.INTER_AREA)

    # crop to frame to only include the road the middle part of the frame
    #frame = frame[300:720, 500:1200]
 ...
 # Changing line color to red if a vehicle in the frame has not crossed the line
    ...
    # for cars in current_detections: if its the first time we are seeing this car add it to
    # the starting point list
    for i in current_detections.items():
        if i[1] not in starting_point:
            starting_point[i[1]] = i[0]

    remove_cars = []
    for i in starting_point:
        if i not in current_detections.values():
           # print("Car ", i, " has left the frame")
            remove_cars.append(i)

    for i in remove_cars:
        del starting_point[i]
        #if i in moving_down:
        #    del moving_down[i]

    on_the_right = [i for i in current_detections.items() if i[0][0] > 750]
    on_the_left = [i for i in current_detections.items() if i[0][0] < 750]

    # for cars on the left if they were on the right before and the y coordinate of the car
    # from the starting point is less than the y coordinate of the car from the current frame
    # then the car has crossed the line
    for i in current_detections.items():
        if i[1] in [p for p in starting_point]:
            if starting_point[i[1]][1] < i[0][1]:
                if i[1] not in moving_down:
                    moving_down.append(i[1])


    #print("Moving down : ", moving_down)
	# if the car is on the left and it is moving down then it has crossed the line
    for i in on_the_left:
        if i[1] in moving_down and i[1] not in flagged_vehicle:
            flagged_vehicle.append(i[1])
            #print("Vehicle ", i[1], " has crossed the line")
			# take the image of the frame and save it
            cv2.imwrite("violations/" + str(i[1]) + ".jpg", frame)
    ...
    previous_right_side.pop(0)
    previous_left_side.pop(0)
    previous_right_side.append(on_the_right)
    previous_left_side.append(on_the_left)
